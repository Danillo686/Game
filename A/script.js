...
const Reiniciar = document.createElement("button") // serve para reiniciar o jogo, será usado só lá embaixo
...
function Jogar() {  // ao apertar em jogar, o user será levado a um pequeno texto explicando como funciona o jogo (tutorial), e todos os botões serão retirados e aparecerá um para o user avançar
    ...
    ModoEscolhido.innerHTML = "Tudo bem! O jogo funciona da seguinte forma: Haverá dois players, você e seu oponente, você deve clicar no botão de dano para tirar um número de 1-6, esse número é o seu dano, seu dever é derrotar o oponente. Boa sorte!"
    ModoEscolhido.style.opacity = "1"
...
function Iniciar() { // quando o user clica pra iniciar o jogo normal, onde sai o texto, e o botão, e entra as barras de vidas, contagem do dano e os atacks
    ModoEscolhido.innerHTML = " " //tirar o texto da frente
    Iniciando.remove()
    ModoEscolhido.innerHTML = `${Jogador1}, você começa`
    ModoEscolhido.opacity = "1"
    
    BarraDoOponente.style.opacity = "1" // aparição das barras de vidas
    BarraDoUser.style.opacity = "1"
    BarraDoOponente.innerHTML = "50"
    BarraDoUser.innerHTML = "50"

    BarraDoOponente.innerHTML = `${VidaDoOponente} ${Jogador2}` //nome dos jogadores, e o total de vida
    BarraDoUser.innerHTML = `${VidaDoUser}  ${Jogador1}`
    
    Dado = document.createElement("button") // botão do jogador 1
    Dado.classList.add ("GiroDoDado1") //estilização
    ...
    DadoOponente.disabled = true //pro jogador 1 iniciar, e o jogador 2 não poder atacar

    Reiniciar.onclick = Resetar //botão de resetar
}
...
function Dano() { // dano do jogador 1
    ...
    BarraDoOponente.innerHTML = `${VidaDoOponente} de vida, Jogador:  ${Jogador2}`  //total de vida e o nome do player

    DadoOponente.disabled = false // para imperdir que os jogadores fiquem roubando os turnos de seus oponentes
    Dado.disabled = true
    ...
    if (VidaDoOponente <= 0){ //quando qualquer jogador ganhar, irá aparecer o botão de reiniciar
        ...
        alinhamento.appendChild (Reiniciar)
    }
}
...
function DanoOponente() { // dano do jogador 2
    ...
    BarraDoUser.innerHTML = `${VidaDoUser} de vida, Jogador:  ${Jogador1}`

    DadoOponente.disabled = true // para imperdir que os jogadores fiquem roubando os turnos de seus oponentes
    Dado.disabled = false
    ...
    if (VidaDoUser <= 0){ //quando qualquer jogador ganhar, irá aparecer o botão de reiniciar
...
function Resetar() { //quando alguém perder, aparecerá o botão para resetar
    location.reload()
}
